...
**`official` 目录已从 `portal` 目录中移出，现在是独立项目。**

新的部署脚本位置：`official/deployment/deploy.sh`

请使用新的部署脚本进行部署。

## 与静态网站部署的区别

- **`deploy.sh`**: 部署 `portal/` 目录下的静态 HTML 网站（index.html, product.html 等）
- **`deploy-official.sh`**: 部署 `portal/official/` 目录下的 Next.js 项目
